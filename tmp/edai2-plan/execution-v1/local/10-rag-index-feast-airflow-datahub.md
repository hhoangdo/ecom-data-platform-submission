...
| Field | Record |
|---|---|
| Status | Complete — local fake-backed storage/query, orchestration, lineage, CAS, compensation, and sentinel Definition of Done achieved. Correction review added a durable actual-DAG callable path, a Feast/SQL relation contract, and full candidate-member embedding validation; no canonical/runtime rubric claim is made. |
...
| Commands / exit codes | `rtk uv run pytest tests/unit/llm/test_indexing.py tests/integration/llm/test_feast_pgvector.py tests/integration/llm/test_airflow_datahub.py -q` — exit `1` before the missing contracts were implemented; original final rerun exit `0` (`23 passed`); correction reruns exit `0` (`26 passed`, then final `27 passed`).<br>`rtk uv run pytest tests/integration/llm/test_feast_pgvector.py -q` — exit `0` (`5 passed`).<br>`rtk uv run pytest tests/unit/llm/test_indexing.py tests/integration/llm/test_feast_pgvector.py -q` — exit `0` (`20 passed`).<br>`rtk uv run pytest tests/integration/llm/test_airflow_datahub.py -q` — exit `0` (`3 passed`, then correction final `6 passed`).<br>`rtk uv run pytest tests/integration/llm/test_airflow_datahub.py -q -k checked_in_dag_callable_uses_serialized_xcom_stage_handoff` — exit `1` (expected red: no runtime handoff).<br>`rtk uv run pytest tests/integration/llm/test_airflow_datahub.py -q -k checked_in_dag_callable_uses_durable_stage_handoff` — exit `1` (expected red: full state in XCom), then exit `0` (`1 passed`) after the durable file-backed handoff.<br>`rtk uv run python scripts/llm/build_index.py --mode candidate --index-version test_idx_001 --index-purpose local-bootstrap-sentinel --source-root data/knowledge/ecommerce --dry-run` — exit `0` (re-run after correction).<br>`rtk git diff --check` — exit `0`.<br>`rtk git diff --cached --exit-code` — exit `0` (no staged delta).<br>`rtk git ls-files --stage` — exit `0` (`911` entries; CRLF-normalized SHA-256 `4f1be432e382df3355b090168be5710c2cb44128a3c03aaf3d71299015a44640`, identical to the pre-topic listing).<br>`rtk git status --short --branch` — exit `0`, branch remains `feature/implement-edai2`. |
| Evidence + SHA-256 | The bootstrap is stdout-only: `test_idx_001`, `dry_run=true`, `evidence_label=local-bootstrap-sentinel`, `storage_written=false`, `promotion=false`, 8 documents, 9 versions, 17 chunks, and 384-dimensional embeddings. No bootstrap report file was retained; emitted `report_sha256` is `a74cd15e7142c3d25432db6390a2f567a4c698c86deea8decaa48468fd8ca1b7`. Correction integration evidence is the final `27 passed` stdout plus the actual checked-in DAG callable test: all nine stages use a JSON handoff stored below its run-scoped local `handoff_root`, while XCom contains only the handoff path. The Feast declaration test proves `edai2_rag_feast_document_feature` supplies the declared fields/timestamp; the storage test proves candidate membership equals declared `chunk_count` and every embedding is 384-dimensional. Locked inputs reverified: Section 03 `3c906ae30ac0fee606e96608a7b5759cd7439ae048c4c12a84511fce5cc33ae6`, EDAI2 plan `b8be3ef5c84fe4d6fe52e8894c3c5dc1c3babc898e2a87684b2b8ff720d6d079`, workbook `71b2403e068081b00245bea5e15c5754f3762ad354e0a3a6576d69e4963c8657`. |
...
| Successor handoff | Topic 11 receives immutable `edai2_rag_*` schema/migrations; exact filtered `<=>` query and Feast/direct equivalence boundary; paused/manual nine-stage DAG contract with file-backed JSON stage handoff and XCom path reference; DataHub candidate/active/read-back/rollback payload contract; CAS compensation to the recorded alias; sentinel identity `test_idx_001`/`local-bootstrap-sentinel`; and the final focused result of `27 passed`. |
