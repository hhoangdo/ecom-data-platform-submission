...
| Field | Record |
|---|---|
| Status | Complete — local fake-backed storage/query, orchestration, lineage, CAS, compensation, and sentinel Definition of Done achieved. Correction review added a durable actual-DAG callable path, a Feast/SQL relation contract, full candidate-member embedding validation, and the active-alias/hash-bound verified-chunk reload boundary required by the locked grounding invariant; no canonical/runtime rubric claim is made. |
| Affected files | `src/vina_bim_shop/llm/indexing.py`<br>`src/vina_bim_shop/llm/ports.py`<br>`src/vina_bim_shop/llm/adapters/feast_postgres.py`<br>`src/vina_bim_shop/llm/adapters/datahub.py`<br>`infra/feast/feature_store.yaml`<br>`infra/feast/features.py`<br>`infra/postgres/edai2/001_extensions.sql`<br>`infra/postgres/edai2/002_knowledge_index.sql`<br>`src/vina_bim_shop/orchestration/rag_index_pipeline.py`<br>`infra/orchestration/airflow/dags/rag_index_pipeline.py`<br>`src/vina_bim_shop/orchestration/specs.py`<br>`infra/governance/recipes/edai2_rag.yml`<br>`scripts/llm/build_index.py`<br>`tests/unit/llm/test_indexing.py`<br>`tests/integration/llm/test_feast_pgvector.py`<br>`tests/integration/llm/test_airflow_datahub.py`<br>`tmp/edai2-plan/execution-v1/local/10-rag-index-feast-airflow-datahub.md` |
| Commands / exit codes | `rtk uv run pytest tests/unit/llm/test_indexing.py tests/integration/llm/test_feast_pgvector.py tests/integration/llm/test_airflow_datahub.py -q` — exit `1` before the missing contracts were implemented; original final rerun exit `0` (`23 passed`); correction reruns exit `0` (`26 passed`, then final `27 passed`, then verified-chunk correction `31 passed`).<br>`rtk uv run pytest tests/integration/llm/test_feast_pgvector.py -q` — exit `0` (`5 passed`, then verified-chunk correction `10 passed`).<br>`rtk uv run pytest tests/unit/llm/test_indexing.py tests/integration/llm/test_feast_pgvector.py -q` — exit `0` (`20 passed`).<br>`rtk uv run pytest tests/integration/llm/test_airflow_datahub.py -q` — exit `0` (`3 passed`, then correction final `6 passed`).<br>`rtk uv run pytest tests/integration/llm/test_airflow_datahub.py -q -k checked_in_dag_callable_uses_serialized_xcom_stage_handoff` — exit `1` (expected red: no runtime handoff).<br>`rtk uv run pytest tests/integration/llm/test_airflow_datahub.py -q -k checked_in_dag_callable_uses_durable_stage_handoff` — exit `1` (expected red: full state in XCom), then exit `0` (`1 passed`) after the durable file-backed handoff.<br>`rtk uv run pytest tests/integration/llm/test_feast_pgvector.py -q -k get_verified_chunk` — exit `1` (`4 failed`, expected red: missing adapter method), then exit `0` (`4 passed`).<br>`rtk uv run python scripts/llm/build_index.py --mode candidate --index-version test_idx_001 --index-purpose local-bootstrap-sentinel --source-root data/knowledge/ecommerce --dry-run` — exit `0` (re-run after corrections).<br>`rtk git diff --check` — exit `0`.<br>`rtk git diff --cached --exit-code` — exit `0` (no staged delta).<br>`rtk git ls-files --stage` — original Topic 10 audit exit `0` (`911` entries; CRLF-normalized SHA-256 `4f1be432e382df3355b090168be5710c2cb44128a3c03aaf3d71299015a44640`, matching its pre-topic listing); verified-chunk correction audit exit `0` (`920` entries; CRLF-normalized SHA-256 `02d22b3ca24661c0df466c40308b8a35d557bd5c03aa6f6d170f5791babfca99`) after prior publication, with no staged delta.<br>`rtk git status --short --branch` — exit `0`, branch remains `feature/implement-edai2`. |
| Evidence + SHA-256 | The bootstrap is stdout-only: `test_idx_001`, `dry_run=true`, `evidence_label=local-bootstrap-sentinel`, `storage_written=false`, `promotion=false`, 8 documents, 9 versions, 17 chunks, and 384-dimensional embeddings. No bootstrap report file was retained; emitted `report_sha256` is `a74cd15e7142c3d25432db6390a2f567a4c698c86deea8decaa48468fd8ca1b7`. Correction integration evidence is the final `31 passed` stdout plus the actual checked-in DAG callable test: all nine stages use a JSON handoff stored below its run-scoped local `handoff_root`, while XCom contains only the handoff path. The Feast declaration test proves `edai2_rag_feast_document_feature` supplies the declared fields/timestamp; the storage test proves candidate membership equals declared `chunk_count` and every embedding is 384-dimensional. The verified-chunk test proves `get_verified_chunk(*, chunk_id, content_sha256)` queries the active alias/candidate membership with both supplied predicates, returns persisted content and immutable citation exactly, and returns `None` for missing, inactive, or hash-mismatched records without vector ranking. Locked inputs reverified: Section 03 `3c906ae30ac0fee606e96608a7b5759cd7439ae048c4c12a84511fce5cc33ae6`, EDAI2 plan `b8be3ef5c84fe4d6fe52e8894c3c5dc1c3babc898e2a87684b2b8ff720d6d079`, workbook `71b2403e068081b00245bea5e15c5754f3762ad354e0a3a6576d69e4963c8657`. |
| Screenshot QA | No screenshot was created. Topic 10 owns no Airflow/DataHub UI, GKE, or canonical screenshot; Topic 28 owns fresh contextual UI proof. |
| Cleanup / runtime release | Tests used injected fake connections/catalogs only; their temporary `handoff_root` is pytest-owned. The compensation test restored the fake active alias from `candidate-1` to its recorded prior `prior-index`. No test schema, Docker container, Kubernetes context, GCP resource, or external runtime was acquired. A real local manual run must use a run-scoped, shared `handoff_root` and remove that run artifact after operational retention requirements are met. |
| Limitations | The host cannot import the PostgreSQL Feast source because psycopg has no C/binary wrapper and no `libpq`; therefore no real PostgreSQL/Feast registration was attempted and no dependency was broadened. The checked-in paused DAG is a local contract: a real manual run requires `dag_run.conf` values for `source_root`, `index_version`, `evaluation_path`, and a shared local `handoff_root`, plus deployment-supplied storage/DataHub adapters; no live scheduler run is claimed. The `test_idx_001` local bootstrap cannot satisfy canonical `Sheet3!E8:E9` evidence. These contracts support E8/E9 only; they do not claim rubric satisfaction. |
| Successor handoff | Topic 11 receives immutable `edai2_rag_*` schema/migrations; exact filtered `<=>` query and Feast/direct equivalence boundary; `FeastPort`/`FeastPostgresAdapter.get_verified_chunk(*, chunk_id, content_sha256) -> SearchMatch | None`, which reloads only a current active candidate member with both SQL predicates and no vector reranking; paused/manual nine-stage DAG contract with file-backed JSON stage handoff and XCom path reference; DataHub candidate/active/read-back/rollback payload contract; CAS compensation to the recorded alias; sentinel identity `test_idx_001`/`local-bootstrap-sentinel`; and the final focused result of `31 passed`. |
