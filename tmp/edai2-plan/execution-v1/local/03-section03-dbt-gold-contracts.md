...
### Follow-up candidate alignment — 2026-08-04

The generator-only point-in-time stream implementation now matches the authoritative latest-cutoff-safe-hour contract already documented in this plan. After the existing lower/upper event-time and created-time predicates, it truncates events to UTC hours, retains each customer's latest hour, and computes stream counts and the ratio from that hour. The new regression test failed before the production change with `f_stream_add_to_cart_60m == 1` and passed after the change with `0`; the focused drift/dbt/Spark contract suite passed `91` tests.

Affected follow-up files are `src/vina_bim_shop/generators/labels.py`, `tests/unit/test_section03_drift.py`, `tests/unit/test_section03_dbt_contract.py`, and `tests/unit/test_spark_batch_runtime.py`. No source-plan contract or config hash changed.

The medium seed-42 regeneration command exited `0` after `845.9` seconds and published candidate bundle `0e889d1e57425e070a87f406e220f758d703326eac67ea1d5f9f58fa2916bb9d`. The new candidate manifest SHA-256 is `9b1fc0b5d9cea1186e173e263e04cd363f65f791110d02345a99dbbe99ae5475`; manifest verification exited `0` with `section03 manifest: PASS (runtime pending)`. The prior `1b4123b312da3d1aca70c2dc4f44cf68db248f3b43aad830e8519ca7bbab7fd3` bundle remains present and recoverable.

The candidate retains `medium`, seed `42`, the locked windows, 11,996 one-to-one training rows, and config SHA-256 `5c5029fea77d93c3941d87d9a60f85cfe374f36899c3d3c1b028f3f5f3c3cd97`. Topic 04 may resume with this candidate; dbt and strict Spark runtime evidence remain separate successor-owned measurements.
