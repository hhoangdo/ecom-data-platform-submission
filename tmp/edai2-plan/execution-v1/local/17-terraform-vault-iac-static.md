...
**Architecture:** The `coursework` Terraform environment composes zonal GKE, Artifact Registry, GCS, KMS, least-privilege IAM, and budget modules. Workload Identity and prefix-conditioned object permissions replace JSON keys. Vault paths/roles are disjoint; only chart/CRD consumers lacking file injection receive exact-key ExternalSecrets.
...
|---|---|---|
| Create | `infra/terraform/environments/coursework/versions.tf` | Terraform/provider version constraints |
| Create | `infra/terraform/environments/coursework/providers.tf` | Provider configuration without credentials |
| Create | `infra/terraform/environments/coursework/variables.tf` | External inputs including safe limits |
| Create | `infra/terraform/environments/coursework/main.tf` | Compose six approved modules |
| Create | `infra/terraform/environments/coursework/outputs.tf` | Non-secret resource IDs only |
| Create | `infra/terraform/environments/coursework/terraform.tfvars.example` | Typed example shape with safe non-secret values and no project identity |
| Generated | `infra/terraform/environments/coursework/.terraform.lock.hcl` | Provider selections generated by backend-disabled init and reviewed/hash-bound |
...
| Create | `configs/gke/profiles.yaml` | suspended/core/rubric-evidence and TTL transitions |
| Create | `configs/gke/cost_envelope.yaml` | $180 forecast, PVC/object/capacity caps |
| Create | `scripts/gke/check_budget.py` | Non-mutating cost-input validation |
...
- [ ] Implement Terraform/config files and run `rtk terraform -chdir=infra/terraform/environments/coursework fmt -check -recursive`. Expected: exit 0; no file changed by check.
- [ ] Run `rtk terraform -chdir=infra/terraform/environments/coursework init -backend=false`. Expected: exit 0 without backend, state, project authentication or mutation.
- [ ] Run `rtk terraform -chdir=infra/terraform/environments/coursework validate`. Expected: exit 0 with configuration valid and no plan/apply.
...
- [ ] Run `rtk uv run python scripts/gke/check_budget.py --help` and `rtk uv run python scripts/gke/manage_profile.py --help`. Expected: exit 0; CLIs expose validation/render modes without contacting GCP.
- [ ] Run `rtk uv run pytest tests/unit/test_edai2_security_static.py tests/unit/test_edai2_repository_contract.py -q` and `rtk git diff --check`. Expected: both exit 0; no tfstate/tfplan/secret file exists.
