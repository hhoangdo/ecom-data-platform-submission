...
|---|---|
| Status | Complete — corrected fail-closed local activation CLI/readback/staging/readiness gaps are covered by the full fake-backed regression. |
| Current branch/status | `feature/implement-edai2...origin/feature/implement-edai2`; final status recorded after Topic 12-only changes, with no staging/index mutation. |
| Affected files | Modified: `src/vina_bim_shop/llm/section03_ingestion.py`, `drift.py`, `adapters/feast_postgres.py`, `api/drift.py`, `mcp/drift.py`, `infra/feast/feature_store.yaml`, `infra/feast/features.py`, `tests/contract/llm/test_api_contracts.py`, `tests/contract/llm/test_mcp_contracts.py`, and this Completion Record. Created: `infra/postgres/edai2/003_section03_features.sql`, `scripts/feast/load_section03.py`, `scripts/llm/smoke_drift.py`, `tests/integration/llm/test_section03_ingestion.py`, and `tests/unit/llm/test_drift.py`. The authorized predecessor-test correction changes the stale Topic 08 `/readyz` assertion to the locked Topic 12 contract: `503 not_ready` until the exact verified active manifest hash is present, then `200`. |
| Commands and exit codes | Expected TDD red checks: focused activation-report, manifest-identity, drift baseline/window/cohort checks, and adapter seams each exited 1 before their minimal implementation. Correction red check for the absent activation CLI/typed adapter seam exited 1. Final `rtk uv lock --check`, strict verify-only CLI, `rtk git diff --check`, and `rtk git diff --cached --exit-code`: 0. Final prescribed local regression `rtk uv run pytest tests/integration/llm/test_section03_ingestion.py tests/integration/llm/test_feast_pgvector.py tests/unit/llm/test_drift.py tests/contract/llm/test_section03_contract.py tests/contract/llm/test_api_contracts.py tests/contract/llm/test_mcp_contracts.py -q`: 68 passed, exit 0 (two third-party deprecation warnings). The structured verify-only CLI emitted the canonical hash, `11996` training rows, `22` health rows, and `rollback_status=not_applicable`. Owned Uvicorn health gate, smoke, and verified listener-PID cleanup: 0; smoke returned allowed local `409 feature_unavailable`. Locked source/workbook SHA-256 commands: 0 and matched all three recorded values. |
| Evidence hashes | `tmp/edai2-local/topic12/drift-response.json` SHA-256 `0b88da9dc5ee421470d8273aba8edf381f365c2ce678ce02329d24e15424b0fe`; non-secret response contains only the typed `feature_unavailable` envelope. The strict verified manifest SHA-256 is `af7189d00a187e539bb777d89111d2860a97822f34c2c911adc3eadf6166f8ad`. |
| Screenshot QA | Not captured locally; no screenshot was manufactured. |
| Cleanup/runtime release | The final owned local listener PID was verified by its exact Uvicorn command line, stopped, waited, and the `127.0.0.1:8082` listener was absent. No Topic 12 test container was created. The two unrelated Docupedia containers were untouched; no Docker prune, Kubernetes, or GCP operation occurred. |
| Limitations | No local PostgreSQL/Feast/Valkey service was acquired. This is non-blocking: the local Definition of Done requires, and has passed, fake-backed staging/readback/no-op/rollback proof. Activation now refuses missing Feast/Valkey readback hooks and refuses incomplete/mismatched same-hash staging. It makes no claim of live `COPY`, live Feast apply, live Valkey materialization, or deployed GKE/MCP evidence. `/readyz` is dependency-aware and returns `503 not_ready` for the absent/wrong hash, declared timeout, or unexpected dependency read failure; the smoke's `409 feature_unavailable` is the allowed no-data response. |
| Handoff | `tmp/edai2-plan/execution-v1/local/13-coordinator-inference-routing-telemetry.md`; consume `SECTION03_MANIFEST_SHA256` / `active_section03_manifest_hash` only after exact equality, and consume the local drift API/MCP contracts. Do not claim deployed MCP/Helm/GKE proof from this topic. |
