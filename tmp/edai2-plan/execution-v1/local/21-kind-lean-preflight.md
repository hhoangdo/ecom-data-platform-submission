...
|---|---|
| Status | Complete — the lean single-node Kind preflight built and loaded the retrieval image, installed the one-slice Helm release, reached a successful Deployment rollout, returned schema-valid HTTP 200 health/readiness responses, captured bounded inventory, and removed every owned runtime artifact. This is local Kind smoke only and earns no GKE rubric credit. |
| Current branch/status | Began clean and synchronized at `b48db37ab69fa542adde072a4813380b76b4df14` on `feature/implement-edai2...origin/feature/implement-edai2`. Current unstaged/untracked delta is limited to the three Kind-only image assets, preflight/runner/test corrections, and this Completion Record. Nothing is staged. |
| Affected files | Added `containers/edai2/{Dockerfile.kind-retrieval,Dockerfile.kind-retrieval.dockerignore,kind-retrieval-requirements.txt}`. Modified `scripts/kind/preflight_edai2_lean.py`, `scripts/kind/run_edai2_lean.ps1`, `tests/unit/test_edai2_kind_preflight.py`, and this record. The production six-target `containers/edai2/Dockerfile`, `pyproject.toml`, `uv.lock`, public API/MCP contracts, Helm chart, and every GCP/GKE path remain unchanged. |
| Commands and exit codes | Fresh locked SHA-256 checks, Topic 20 predecessor check, `rtk uv lock --check`, preflight, PowerShell parsing, `rtk git diff --check`, and cleanup checks exited 0. TDD red/green corrections cover the lean image boundary, strict native-command handling, CRI image verification, and numeric non-root runtime identity; the Topic 21 suite reached 11 passed. The lean Docker build and isolated application import exited 0. The successful final runner exited 0 in 51 seconds: Kind created `edai2-lean` from the pinned node image, loaded `edai2/retrieval-agent:kind-local`, verified it through CRI `ListImagesResponse.images[]`, received API `/readyz = ok`, applied the bounded namespace controls, installed Helm release `edai2-retrieval-kind`, reported `deployment "edai2-retrieval-kind" successfully rolled out`, captured both probes and runtime inventory, uninstalled the release, and deleted the owned cluster. Earlier failed attempts were diagnostic and each reached exact-owner cleanup; the final blocker was kubelet rejecting the image's named `USER edai2` under `runAsNonRoot`, fixed test-first by declaring numeric `USER 10001:10001`. |
| Evidence hashes | `tmp/edai2-kind/preflight.json` SHA-256: `3f01c99800a8c5fb3269acfbe4620ddbd247976b53e2a0082b89ae4b177948a3`; `tmp/edai2-kind/inventory.json`: `7dc0cf97076f14efe80b93d73de557f76198c004d101aaaeca9d13d51e92809c`; `tmp/edai2-kind/health.json`: `3f6489ae37493c858befc67bb409ca685888fadfd81bbf20d35509dc98fc8a55`; `tmp/edai2-kind/readiness.json`: `56b343d8c46b28243ba8c3661e2793ee5739a57fe19a890116bbb98e8800c95`. Probe status files both contain `200`. |
| Screenshot QA | None; Kind is not GKE evidence |
| Cleanup/runtime release | Fresh `rtk kind get clusters` reports no Kind clusters; the dedicated kubeconfig and port-forward PID are absent (`KIND_CLEANUP=PASS`). The Helm release was uninstalled before cluster deletion. Evidence files remain, but no namespace, release, cluster, kubeconfig, or port-forward process remains. Six externally managed Docupedia containers were present at final inspection; the topic issued no stop/remove/prune command and does not claim their count or IDs are lifecycle-stable. |
| Limitations | Local Kind only: no GKE, llm-d, Jenkins, observability, Vault, screenshot, or Sheet3 evidence. The successful inventory contains exactly one Running retrieval Pod, zero PVCs, and one ClusterIP Service; preflight totals are one Pod, 100m/256Mi requests, 500m/512Mi limits, zero PVC/storage, and zero LoadBalancer. The Kind-only image is 51,458,574 bytes, declares numeric `10001:10001`, and has final local image ID `sha256:ce0a1c242f28e64a9f8a348ccb929240e5e0ce1afde25964ebb1f7db6b4db4e3`; BuildKit attestation means its manifest-list ID may change on a rebuild even when the content layers are cached. |
