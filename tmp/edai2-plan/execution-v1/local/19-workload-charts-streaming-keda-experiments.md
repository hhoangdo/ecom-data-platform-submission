...
**Architecture:** Topic 18 charts render six commit-SHA releases. Kafka bootstrap precedes writers. The drift Jenkins/Helm release alone invokes the Topic 12 loader after verified upload; no duplicate importer exists. CI bootstrap retrieval index is explicitly temporary and distinct from the canonical Airflow evidence index. Experiment configurations hold all non-tested factors constant and fail closed on promotion.
...
|---|---|---|
| Create | `infra/helm/edai2/workloads/retrieval.yaml` | Retrieval service/MCP/support agent commit-SHA release |
...
| Create | `src/vina_bim_shop/llm/streaming/offline_writer.py` | Offline destination/idempotency/group/DLQ |
| Create | `src/vina_bim_shop/llm/streaming/online_writer.py` | Online destination/idempotency/group/DLQ |
| Create | `scripts/feast/run_offline_writer.py`, `scripts/feast/run_online_writer.py` | Thin worker CLIs |
| Create | `configs/llm/warmup_prompts.json` | Three fixed shared-prefix warmups |
| Create | `configs/llm/benchmark_requests.json` | Fixed ordered 40-request mixed-prefix fixture |
...
CI bootstrap index is exactly `ci-bootstrap-${GIT_COMMIT}`, temporary and sufficient only for release smoke. `GIT_COMMIT` is the full lowercase 40-hex value from `rtk git rev-parse --verify HEAD`; the workload/CI validator rejects a non-40-hex value or a value different from the Jenkins checkout revision. Canonical evidence index is created/promoted later by the Airflow RAG pipeline with its own version/hash/evaluation lineage; it is never relabeled from CI bootstrap and requires no image rebuild.
...
- [ ] Render retrieval with `rtk helm template retrieval infra/helm/edai2/service-agent -f infra/helm/edai2/workloads/retrieval.yaml --set image.tag=testsha`. Expected: exit 0, commit sentinel, no LoadBalancer, bounded KEDA.
- [ ] Render drift with `rtk helm template drift infra/helm/edai2/service-agent -f infra/helm/edai2/workloads/drift.yaml --set image.tag=testsha`. Expected: exit 0 and exactly one activation owner referencing verified loader.
- [ ] Render coordinator with `rtk helm template coordinator infra/helm/edai2/service-agent -f infra/helm/edai2/workloads/coordinator.yaml --set image.tag=testsha`. Expected: exit 0 and exactly three internal destinations/one facade.
- [ ] Render both writers with `rtk helm template feast-offline-writer infra/helm/edai2/worker -f infra/helm/edai2/workloads/feast-offline-writer.yaml --set image.tag=testsha` and `rtk helm template feast-online-writer infra/helm/edai2/worker -f infra/helm/edai2/workloads/feast-online-writer.yaml --set image.tag=testsha`. Expected: both exit 0 with distinct groups and no import/bootstrap duplication.
