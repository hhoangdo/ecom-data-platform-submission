...
|---|---|
| Status | Complete |
| Current branch/status | `feature/implement-edai2...origin/feature/implement-edai2`; the final shared live-GKE option correction modifies only `tests/conftest.py`, `tests/e2e/llm/conftest.py`, `tests/integration/llm/conftest.py`, and this Topic 20 record; `rtk git diff --cached --quiet` exited 0, so no staged entries were created. |
| Affected files | Created `infra/observability/prometheus/alerts.yaml`, `infra/observability/alloy/config.alloy`, `infra/observability/grafana/dashboards/{http,compute,agents,llm,ab}.json`, `infra/ingress/edai2/{ingresses,certificate-issuer}.yaml`, `scripts/gke/configure_evidence_ingress.py`, `scripts/qa/capture_edai2_evidence.py`, `tests/unit/llm/test_observability.py`, `tests/e2e/llm/test_required_uis.py`; corrected the single global live-GKE option registration in `tests/conftest.py` and removed duplicate registrations from `tests/e2e/llm/conftest.py` and `tests/integration/llm/conftest.py` while retaining the integration live fixture. `pyproject.toml` and `uv.lock` remain unchanged because Pillow was already declared and locked. |
| Commands and exit codes | Fresh locked SHA-256 checks for the two source plans and rubric exited 0; the Topic 19 predecessor is `Complete`. Fresh `rtk uv lock --check`, `rtk git diff --check`, and `rtk git diff --cached --quiet` exited 0. Correction TDD red gates exited 1 as expected: 4 failures/5 passes/1 live-GKE skip for exact anchors, signed inventory, lifecycle, and capture rejection; 1 failure for the direct telemetry validator; then 3 failures for all-entry inventory validation, existing-manifest validation, and selected-issuer ordering. Their green gates exited 0. Final focused suite exited 0 (20 passed, 1 skipped), prescribed inference/coordinator regression exited 0 (54 passed, 1 skipped), render-only ingress CLI with explicit render-only kubeconfig/context exited 0, and static capture verifier exited 0. An initial non-installing Helm render without a repository reference failed only because the repo was not configured; the controller's approved transient client-only retry `rtk helm --kubeconfig tests/fixtures/kubernetes/render-only-kubeconfig.yaml --kube-context render-only template ingress-nginx ingress-nginx --repo https://kubernetes.github.io/ingress-nginx --version 4.15.1 -f infra/helm/edai2/values/ingress-nginx.yaml --set controller.replicaCount=1` exited 0 without Kubernetes API contact. Final collection correction: initial `rtk uv run pytest --collect-only -q` exited 1 with duplicate `--live-gke`; after moving registration to the common parent it exited 0 (908 collected). The Topic 20 focused regression exited 0 (54 passed, 1 skipped); independent integration live-fixture and E2E suites exited 0 (7 passed; 11 passed, 1 skipped). Full `rtk uv run pytest -q` completed (899 passed, 1 skipped, 8 failed): all eight failures were pre-existing unrelated Kafka/DAG/scope/script-inventory expectations, while the former collection error was absent. |
| Evidence hashes | Locked source-plan SHA-256 values were freshly verified as `3c906ae30ac0fee606e96608a7b5759cd7439ae048c4c12a84511fce5cc33ae6` and `b8be3ef5c84fe4d6fe52e8894c3c5dc1c3babc898e2a87684b2b8ff720d6d079`; rubric SHA-256 was freshly verified as `71b2403e068081b00245bea5e15c5754f3762ad354e0a3a6576d69e4963c8657`. No final evidence artifact or screenshot was generated. |
| Screenshot QA | No live screenshots were captured. Synthetic PNG validation uses Pillow signature/full-decode, 1600x1000 dimensions, non-uniform-content rejection, manifest hash, and atomic replacement; later owner must use original-resolution `view_image`. |
| Cleanup/runtime release | Synthetic tests use pytest temp paths. The injected private-capture lifecycle verifies canonical inventory integrity and every entry's UID/port/selector/EndpointSlice list before port reservation or process creation; it forwards a reserved non-privileged local port to the inventory service port, records only stable endpoint key/service UID source metadata, and terminates/waits/polls the exact fake child on success, timeout, capture error, interruption, start failure, terminate/wait failure, and leak detection. Existing manifest entries are fully validated for safe path/source, exact anchors, dimensions, bounded aware timestamp, current revision, PNG existence, and hash before a replacement can publish. The ingress enable contract atomically renders and applies only the selected Ready `ClusterIssuer` before its owned routes; disable deletes only those routes and leaves the shared issuer/controller untouched. It removes only owned PID/stdout/stderr/runtime and capture/manifest temporary or rollback files; the explicit Topic 20 temp-root check was empty. No real tunnel, ingress, certificate, Kubernetes object, GCP resource, browser capture, Docker stop, or prune occurred. |
| Limitations | Complete for the local/static contract only. All contextual UI/TLS/ACME/auth-rate/alert-transition/dashboard-data/log/trace/Langfuse/A-B and Terraform-apply proof remains deferred to designated GCP topics. The approved Helm operation was a transient client-only template render, not an install or cluster contact; local render-only use of the explicit `render-only` kubeconfig/context is not GKE evidence. |
