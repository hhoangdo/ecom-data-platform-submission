...
|---|---|
| Status | Complete |
| Current branch/status | `feature/implement-edai2`; final `rtk git status --short --branch` showed only Topic 16 changes: modified `tests/unit/test_edai2_repository_contract.py` and this plan record; untracked `ci/`, `containers/`, `infra/helm/edai2/service-agent/`, and `infra/helm/edai2/worker/`; no staged entries. The verified raw `git ls-files --stage` UTF-8/LF listing SHA-256 was and remained `80020b956061a4dcb8fe4d6db57ca5fae3ada2ba50f29a4d306deeb0425cbfdc`; the earlier `b742...` recording used a different hash method. |
| Affected files | Created the Topic 16 Docker/catalog, Jenkins jobs/pod/change-map/pipeline/six Jenkinsfiles/release selector, and reusable `service-agent`/`worker` charts; added the operational `containers/edai2/Dockerfile.dockerignore` for root-context Dockerfile-specific exclusions; modified `tests/unit/test_edai2_repository_contract.py`; updated only this Completion Record among plan records. |
| Commands and exit codes | Preconditions: `rtk git status --short --branch`, all three `rtk proxy certutil -hashfile ... SHA256` checks, predecessor status lookup, `rtk uv lock --check`, and `rtk git diff --check` all exited 0. Red gate: `rtk uv run pytest tests/unit/test_edai2_repository_contract.py -q` exited 1 with the expected absent Topic 16 path/selector failures. Focused green gates: Docker/image, Jenkins/BuildKit/stage, change-map, and Bash parser contract commands all exited 0. Correction red gate for release metadata exited 1 as expected; its focused Jenkins/BuildKit/stage and Bash parser reruns exited 0. Dispatcher correction red gate (`rtk uv run pytest tests/unit/test_edai2_repository_contract.py -q -k "release_script_has_valid_bash_syntax"`) exited 1 with `release.sh: line 68: $2: unbound variable`; its executable fake-Helm green rerun and `rtk proxy bash -n ci/jenkins/scripts/release.sh` both exited 0. Final static gate: `rtk uv run pytest tests/unit/test_edai2_repository_contract.py tests/unit/test_edai2_security_static.py -q` exited 0 (`9 passed`). Six `rtk proxy helm --kubeconfig tmp/edai2-kind/kubeconfig --kube-context kind-edai2-lean template ...` renders exited 0 without API contact, including the explicit post-correction rerun. |
| Evidence hashes | Sentinel render SHA-256: retrieval `35538846a2e34f4d179bce9d8d85fa0a7b7b8f9a7b5fa9db6b5c857de8a3ddb5`; drift `5f205fe417e9815b67eea47e7a52ae31cb0233f16a011b62c08bf9bd6a04939d`; coordinator `f6d3a3b742bf0a391adb3b044e93ba2873b3a1362caf166b13e1800af5a0e31a`; rag-index `9cbd64112608de4b7a88a72b5894ef0bfb1f797f512fc6c9927089f43b93db0d`; feast-offline-writer `20f2907da40ada56d90d75d453c662bc43db5f37c39687d965d324bf42cd5dfb`; feast-online-writer `2f93b47a5bb8cf0e6d83d89b66888c0413c385cbf53edbab9f8102498888b8c7`. Machine terminal output is authoritative. |
| Screenshot QA | No screenshot manufactured or captured. Six successful Jenkins job/build-page captures remain deferred to the later runtime evidence owner. |
| Cleanup/runtime release | Render-only files were written below `tmp/edai2-topic16-render/` for hashing and removed after recording the six hashes. No Docker build/image, Jenkins runtime, Helm release, Kubernetes API action, GCP action, or running process was created. The explicit kubeconfig/context flags guarded the existing corporate default context. |
| Limitations | Static contracts and client-only renders do not prove a real archive, scan result, Artifact Registry digest, Workload Identity session, Jenkins execution, Helm deployment, smoke/evaluation, rollback, or Sheet3 screenshot. The Topic 19-owned writer CLIs are recorded as future image commands but are not executed here. |
| Handoff | `tmp/edai2-plan/execution-v1/local/17-terraform-vault-iac-static.md` consumes the CI security boundary; Topics 18/19 may consume/lint/render but must not edit Topic 16 chart templates. |

## Correction Addendum — Topic 19 reusable interface repair

This addendum preserves the `Complete` status above. It corrects only Topic 16-owned reusable interfaces required for Topic 19's authored workload values; it creates no Topic 19 workload values or streaming artifacts.

| Field | Correction evidence |
|---|---|
| Scope | `service-agent` and `worker` now accept optional KEDA polling/cooldown/trigger values and optional deployment strategy, readiness/liveness probe, and resource values without changing legacy renders. `release.sh` now accepts an optional ordered second values file for Topic 19's future atomic call-site update. The six checked-in Jenkinsfiles intentionally remain self-contained on their existing one-file calls until the Topic 19 workload files are created. |
| CI fail-closed contract | The executable fake-Helm test proves exact ordered `-f` arguments, repository, commit tag, workload name, explicit kubeconfig/context, unknown mapping rejection, and required service substrate bucket rejection before Helm runs. |
| Fresh commands and exit codes | Red gate: `rtk uv run pytest tests/unit/test_edai2_repository_contract.py tests/integration/llm/test_gke_agents.py -q -k "reusable_charts_render_topic19_keda_and_deployment_interfaces or existing_topic16_chart_values_still_render_without_topic19_overrides or topic19_release_values_are_ordered_and_fail_closed_in_helm_atomic or model_configs_workerpool_and_cpu_overlay_are_private_and_bounded"` exited 1 with the expected absent strategy/second `-f`/trigger failures. Green rerun exited 0 (`4 passed`). `rtk proxy bash -n ci/jenkins/scripts/release.sh` 0; both `rtk helm lint` chart commands 0; `rtk uv run pytest tests/unit/test_edai2_repository_contract.py tests/integration/llm/test_gke_agents.py tests/unit/test_edai2_security_static.py -q` 0 (`22 passed`). |
| Fresh render hashes | Windows-host UTF-8/LF stdout SHA-256: legacy retrieval render `e26509a42ef2c61646817ba8d3eb1e57da347b54d0075fdef0f31493ea9b0987`; legacy worker render `d2c9bccbcc3bdd6953d6ad9c0a99f84a598c0f502677aef2a69e51586e65cf9e`. |
| Limitations | These are static render and fake-Helm contracts only. No Topic 19 workload file, Jenkins execution, Helm release, Kubernetes API action, build, push, registry, Docker, GCP, or runtime scaling was performed. |
| Review correction evidence | Red rerun: `rtk uv run pytest tests/unit/test_edai2_repository_contract.py -q -k "topic16_jenkins_image_buildkit_stage_chart_contracts or topic19_release_values_are_ordered_and_fail_closed_in_helm_atomic"` exited 1 with the expected future-file call-site and legacy-worker-plus-extra-file failures. Green rerun exited 0 (`2 passed`). Worker validation now accepts exactly one values file, either its expected future workload path or `infra/helm/edai2/worker/values.yaml`; no extra file reaches Helm. |
