...
10. Every GCP topic first runs the redacted, non-mutating `check_budget.py --live-external-preflight` gate for project lifecycle, billing link, exact IAM permissions, trial expiry, current spend/forecast, notification target, approved recovery sink, DNS, and topic-specific external inputs. Stop before mutation if any gate fails; never emit raw project, billing-account, principal, sink, credential, URI, or URL values.
11. Every live gate writes immutable topic-specific artifacts: `gcp_preflight_topicNN.json` and `cost_forecast_topicNN.json`; Topic 30 uses distinct `_recovery` and `_resume` pairs. A successor may read and hash these files but never overwrite them. `usage_ledger.json` is the sole shared append-only GCP gate artifact.
12. `tmp/edai2-gcp/coursework.auto.tfvars` stays untracked. `EDAI2_TFVARS_PATH` must be an absolute path resolving exactly to that repository file so Terraform `-chdir` cannot rebase it.
13. Machine-readable evidence governs. After one bounded tuning retry, failed empirical gates remain Partial/Missing; compatible maximum is 99 because E49 is permanently Out of Scope.
14. Topic 08 creates the EDAI2 Python module/config/test scaffolds and baseline dependencies. Later topics modify those paths, fail closed if a required baseline dependency is absent, and add only the literal dependency explicitly assigned to that topic.
15. Topic 16 is the sole owner of reusable `service-agent` and `worker` chart templates. Topic 14 owns app-agent values, Topic 19 owns workload values, Topic 18 only lints/consumes the charts, and final service renders merge the corresponding agent and workload values files.
16. Every local API, port-forward, or browser helper starts hidden, records its exact PID and logs beneath its topic-owned `tmp/` directory, waits for readiness, and stops and verifies that exact PID before completion.
17. Live GCP topics consume the implementation completed in Topics 08-20. If a source/IaC/chart defect appears, capture the failure, disable routes, release or suspend the owned runtime, mark the live topic `Partial`, and return the defect to its local owner; do not spend an active cloud lease patching repository implementation.
...
- The Kubernetes resource named `support` has the logical identity `retrieval`; registry, labels, environment, and evidence use `retrieval`.
- Agent substrate object prefixes derive from the required non-empty Helm value `substrate.bucketName`; values files never contain shell-substitution placeholders.
...
| 25 | [`gcp/25-jenkins-six-workload-deploy.md`](gcp/25-jenkins-six-workload-deploy.md) | Live GCP | GCP mutation | 24 | EDAI2 Task 9 six CI/CD deployments, verified Section 03 pre-upload with drift-owned activation, and noncanonical CI-bootstrap RAG | E10-E12, E16-E18 |
